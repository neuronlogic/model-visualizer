...
host.BrowserHost = class {
    ...
    formatNumber(num) {
        if (num >= 1e6) {
            ...
            return num.toLocaleString();  // For values less than 1,000
        }
    }
    setProgress(percent) {
        ...
        // Update the inside text with the percentage
        insideCircle.textContent = `${percent}%`;

        const angle = (percent / 100) * 360;

        if (percent <= 50) {
            circle.querySelector('.mask.full').style.transform = `rotate(0deg)`;
            circle.querySelector('.mask.half .fill').style.transform = `rotate(${angle}deg)`;
            circle.querySelector('.mask.half .fill.fix').style.transform = `rotate(0deg)`;
        } else {
            circle.querySelector('.mask.full').style.transform = `rotate(${angle - 180}deg)`;
            circle.querySelector('.mask.half .fill').style.transform = `rotate(180deg)`;
            circle.querySelector('.mask.half .fill.fix').style.transform = `rotate(${angle}deg)`;
        }
    }
